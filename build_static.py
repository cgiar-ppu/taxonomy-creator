...
function buildSunburst() {
    ...
    .then(function(data) {
        ...
        // Segment labels group
        var labelGroup = svg.append('g').attr('class', 'sunburst-labels').style('pointer-events', 'none');

        function updateSegmentLabels(target) {
            labelGroup.selectAll('text').remove();
            var targetX0 = target.x0;
            var targetX1 = target.x1;
            var targetSpan = targetX1 - targetX0;
            var xScale = d3.scaleLinear().domain([targetX0, targetX1]).range([0, 2 * Math.PI]);
            var yScale = d3.scaleLinear().domain([target.y0, radius]).range([0, radius]);
            ...
            labellable.forEach(function(d) {
                var midAngle = xScale((d.x0 + d.x1) / 2);
                var midRadius = (yScale(d.y0) + yScale(d.y1 - 1)) / 2;
                var x = midRadius * Math.sin(midAngle);
                var y = -midRadius * Math.cos(midAngle);
                var rotDeg = (midAngle * 180 / Math.PI) - 90;
                var flip = midAngle > Math.PI;
                if (flip) rotDeg += 180;
                var arcLen = (d.x1 - d.x0) / targetSpan * 2 * Math.PI * midRadius;
                var maxChars = Math.max(3, Math.floor(arcLen / 6));
                var label = d.data.name || '';
                if (label.length > maxChars) label = label.substring(0, maxChars - 1) + '..';
                labelGroup.append('text')
                    .attr('x', x).attr('y', y)
                    .attr('transform', 'rotate(' + rotDeg + ',' + x + ',' + y + ')')
        ...
        function zoomTo(target) {
            ...
            setTimeout(function() { updateSegmentLabels(target); }, 780);
        }

        // Initialize nav bar and labels for root view
        updateNavBar(hierarchy);
        updateSegmentLabels(hierarchy);
